...
if __name__ == "__main__":
    ...
    a = np.load("a.npy")
    b = np.load("b.npy")
    a = torch.from_numpy(a)[:M*K].reshape(-1, K).cuda()
    b = torch.from_numpy(b)[:N*K].reshape(-1, K).cuda()
    ...
    print(c_soft)
    # Check the result
    # expected_result = torch.matmul(a.view(torch.float8_e4m3fn).to(torch.float16), b.view(torch.float8_e4m3fn).transpose(0, 1).to(torch.float16))
    # load expected_result from npy
    expected_result = np.load("expected_result_chitu.npy")
    expected_result = torch.from_numpy(expected_result)[:M*N].reshape(-1, N).cuda()
    ...
    # # write c_soft to npy
    # np.save("c_soft_chitu.npy", c_soft.cpu().numpy())
    # # write expected_result to npy
    # np.save("expected_result_chitu.npy", expected_result.cpu().numpy())
